...
async def get_products(
    ...
    # Получаем имена фермеров и рейтинги
    product_responses = []
    for product in products:
        ...
        product_responses.append(ProductResponse(
            ...
            fermer_rating=fermer.bonus_points,  # proxy для рейтинга
            title=product.title,
...
async def get_product(product_id: int, db: AsyncSession = Depends(get_db)):
    ...
    # Если товар неактивен, показываем только автору или админу
    if product.status != ProductStatus.ACTIVE:
        # Временно пропускаем проверку для MVP
        pass
...
async def create_product(
    ...
    # Считаем активные товары фермера
    ...
    new_product = Product(
        ...
        status=ProductStatus.PENDING  # На модерации
    )
    db.add(new_product)
    await db.commit()
    ...
    for i, photo in enumerate(valid_photos[:10]):  # максимум 10 фото
    ...
    # Начисляем бонус за добавление товара (+10 баллов)
