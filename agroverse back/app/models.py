...
class User(Base):
    ...
    role = Column(Enum(UserRole, values_callable=lambda x: [e.value for e in x]), default=UserRole.XARIDOR)
    tariff = Column(Enum(UserTariff, values_callable=lambda x: [e.value for e in x]), default=UserTariff.STANDART)
...
class Product(Base):
    ...
    status = Column(Enum(ProductStatus, values_callable=lambda x: [e.value for e in x]), default=ProductStatus.PENDING)
...
class Order(Base):
    ...
    pickup_method = Column(Enum(PickupMethod, values_callable=lambda x: [e.value for e in x]), default=PickupMethod.SELF)
    status = Column(Enum(OrderStatus, values_callable=lambda x: [e.value for e in x]), default=OrderStatus.CREATED)
    created_at = Column(DateTime, server_default=func.now())
