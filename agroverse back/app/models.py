...
class User(Base):
    ...
    role = Column(String(20), default=UserRole.XARIDOR.value)
    tariff = Column(String(20), default=UserTariff.STANDART.value)
    ...
    orders_as_xaridor = relationship("Order", foreign_keys="Order.xaridor_id", back_populates="xaridor", viewonly=True)
    orders_as_fermer = relationship("Order", foreign_keys="Order.fermer_id", back_populates="fermer", viewonly=True)
...
class Order(Base):
    ...
    delivery_request = relationship(
        ...
        uselist=False,
        foreign_keys=[delivery_request_id]
    )
...
class DeliveryRequest(Base):
    ...
    order   = relationship(
        "Order",
        back_populates="delivery_request",
        foreign_keys="Order.delivery_request_id"
    )
